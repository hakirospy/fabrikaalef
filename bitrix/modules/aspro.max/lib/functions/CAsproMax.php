...
if(!class_exists("CAsproMax"))
{
	class CAsproMax{
		...
		public static function showItemCounter($arAddToBasketData, $id, $arItemIDs, $arParams = array(), $class = '', $svgSize = '', $bWrap = false){
					...
					foreach($arItems as $key => $arItem2)
					{
						if($arItem2['~PROPERTY_LINK_GOODS_FILTER_VALUE'])
						{
							...
							if(
								...
								$arParamsTmp['CUSTOM_FILTER'] = $arItem2['~PROPERTY_LINK_GOODS_FILTER_VALUE'];
		...
		public static function checkAvailable($arFilter, $options = array()) {
		...
		public static function showBottomPanel() {
			...
			if ($arTheme['BOTTOM_ICONS_PANEL']["VALUE"] == 'Y') {
				...
				if ($iblockID) {
					...
					$arItems = \CMaxCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'ID' => 'ASC', 'CACHE' => array('TAG' => \CMaxCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), $arFilter, false, false, array('ID', 'NAME', 'IBLOCK_ID', 'PROPERTY_IMG', 'PROPERTY_TYPE', 'PROPERTY_URL', 'PROPERTY_SHOW_TEXT'));?>

...
class IblockPropertyBuildControls extends \CCatalogCondCtrlIBlockProps implements BuildControlsInterface
{
    ...
    public static function getControls($controlId = false, $filter = [])
    {
        ...
        $iblocks = self::getIblockIds($filter);
    ...
    protected static function getIblockIds($filter = [])
    {
    ...
    protected static function getPropertiesByIblockId($iblockId)
    {
        ...
        if(self::$params['ONLY_PROPERTY_SMART_FILTER'] === true) {
            ...
            $sectionProperties = array_column($rows, 'DISPLAY_TYPE', 'PROPERTY_ID');
        }
        ...
        $propertyList = \Bitrix\Iblock\PropertyTable::getList(array_filter([
              ...
              'filter' => array_filter([
                  'IBLOCK_ID' => $iblockId,
                  'ID' => $sectionProperties ? array_keys($sectionProperties) : [],
                  '!PROPERTY_TYPE' => 'F',
              ]),
              'order' => [
                  'NAME' => 'ASC',
        ...
        foreach ($propertyList as $property) {
            ...
            $property['DISPLAY_TYPE'] = $sectionProperties[$property['ID']];
            $result[] = $property;
        }
    ...
    protected static function getActualPropertyIds($iblockId, $sectionIds)
    {

        $sections = self::getSections($sectionIds);

    }

    protected static function getSections(array $sectionIds)
    {
        if(!$sectionIds) {
            return false;
        }

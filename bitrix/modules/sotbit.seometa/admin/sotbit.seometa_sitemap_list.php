...
// Sites list
$arSites = array();
$rsSites = CSite::GetList($by1 = "NAME", $order1 = "desc", array("ACTIVE" => "Y"));
...
<script>
    ...
    BX.runSitemap = function (ID, value, pid, NS, iteration = 0, sectionsArray = []) {
        ...
        BX.ajax.post('/bitrix/admin/sotbit.seometa_sitemap_run.php', {
            lang: '<?=LANGUAGE_ID?>',
            action: 'sitemap_run',
            currentSection: sectionsArray[iteration],
            ...
            // if (sectionsArray.length > 0){
            //     BX.runSitemap(ID, 0, '', '', iteration, sectionsArray);
            // } else {
                BX.adminPanel.closeWait(BX('sitemap_run_button_' + ID));
            // }
        });
    ...
    BX.checkStatus = function (ID) {
        ...
        BX.ajax.post('/bitrix/admin/sotbit.seometa_sitemap_run.php', {
            ...
            if(data.indexOf('stop') == -1 && data !== undefined) {
                ...
                BX.checkStatus(ID);
                // setTimeout(function(){BX.checkStatus(ID)},20)
            }
            else
            {

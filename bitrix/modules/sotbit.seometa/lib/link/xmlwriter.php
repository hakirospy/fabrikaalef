...
class XmlWriter extends AbstractWriter
{
    ...
    private function setCountWrited($reset = true) {
        ...
        $this->allCountWrited++;
        file_put_contents($_SERVER['DOCUMENT_ROOT']. '/seometa_data_tmp.txt', $this->allCountWrited);
    }
    ...
    public function writeMainSiteMap() {
        ...
        if($siteMapLink && file_exists($siteMapLink)) {
            ...
            for($i = 0; $i < count($xml->sitemap); $i++)
            {
                $k = 0;
                do {
                    if (isset($xml->sitemap[$i]->loc) && $xml->sitemap[$i]->loc == $this->siteUrl . '/sitemap_seometa_' . $this->id . '_' . $k . '.xml') {
            ...
            if(!$FoundSeoMetaSitemap) // if sitemap_seometa is not found then add it to main sitemap
            {
                ...
                do {
                    ...
                    $NewSitemap->addChild("loc", $this->siteUrl . '/sitemap_seometa_' . $this->id . '_' . $k . '.xml');
            ...
            if(file_exists($_SERVER['DOCUMENT_ROOT']. '/seometa_data_tmp.txt')) {
                unlink($_SERVER['DOCUMENT_ROOT']. '/seometa_data_tmp.txt');
            }
        }
    }

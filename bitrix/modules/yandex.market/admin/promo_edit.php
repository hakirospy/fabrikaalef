...
else
{
	...
	if (!empty($_GET['id']))
    {
        ...
        $contextMenu[] = [
            'LINK' => '/bitrix/admin/yamarket_promo_result.php?lang=' . LANGUAGE_ID . '&find_element_id=' . $id . '&set_filter=Y',
            'TEXT' => Market\Config::getLang('PROMO_EDIT_CONTEXT_MENU_EXPORT_RESULT')
        ];

        $contextMenu[] = [
            'LINK' => '/bitrix/admin/yamarket_log.php?lang=' . LANGUAGE_ID . '&find_promo_id=' . $id . '&set_filter=Y',

...
class Temporary extends Base
{
	...
	protected function removeQuotes($fieldName, Main\DB\Connection $connection = null)
	{
		...
		if ($sqlLeftQuote !== '' && strpos($fieldName, $sqlLeftQuote) === 0)
		{
			$sqlLeftQuoteLength = strlen($sqlLeftQuote);
			$sqlRightQuoteLength = strlen($sqlHelper->getRightQuote());

			$result = substr($fieldName, $sqlLeftQuoteLength, -1 * $sqlRightQuoteLength);
		}
		else
		{
			$result = $fieldName;

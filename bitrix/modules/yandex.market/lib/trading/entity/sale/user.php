...
class User extends Market\Trading\Entity\Reference\User
{
	...
	{
		$delimiterPosition = strpos($email, '@');
		$randomizer = '_' . randString(5);

		if ($delimiterPosition !== false)
		{
			$result =
				substr($email, 0, $delimiterPosition)
				. $randomizer
				. substr($email, $delimiterPosition);
		}
		else
		{
	...
	protected function randomizePhone($phone)
	{
		...
		{
			$symbol = substr($phone, $symbolIndex, 1);
			...
			if (preg_match('/\d/', $symbol))
			{
				...
				$phone =
					substr($phone, 0, $symbolIndex)
					. $randomDigit
					. substr($phone, $symbolIndex + 1);
